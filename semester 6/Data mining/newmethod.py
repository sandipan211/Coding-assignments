# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 11:24:54 2018
 ...
 while(len(simtype)>c):
  ...
  for i in range(len(simtype)):
    for j in range(len(simtype)): 
       if set(simtype[i]).issubset(set(simtype[j]))==True and i!=j:
         newlist.append(simtype[j])
         break
  simtype = newlist
  ...
  for i in range(len(simtype)):
      for j in range(i+1, len(simtype)):
          if simclus[i,j]==maxval:
              ...
              maxclus.append(((x, i, j)))
              
  newunion = random.choice(maxclus)
  newunion = newunion[0]
  x = newunion[1]
  y = newunion[2]
  indices = x, y
  simtype = [i for j, i in enumerate(simtype) if j not in indices]
  simtype.append(newunion)
...
with open('E:/Academics/sem 6 docs/Data mining/assignment 1/iris.csv') as file:
