# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 11:11:21 2018
...
for i in range(0,150):
    ...
    d_row_avgs[i] = ((150*(np.mean(dice[i])))-dice[i][i])/149
    j_row_avgs[i] = ((150*(np.mean(jaccard[i])))-jaccard[i][i])/149
    c_row_avgs[i] = ((150*(np.mean(cosine[i])))-cosine[i][i])/149
    o_row_avgs[i] = ((150*(np.mean(overlap[i])))-overlap[i][i])/149
